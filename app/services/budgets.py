...
def spent_for_category(db: Session, user_id: int, category_id: int,
...
def set_budget(db: Session, *, user_id: int, category_id: int, amount: int,
    ...
    if existing:
        ...
        db.refresh(existing)
        return existing
    budget = Budget(user_id=user_id, category_id=int(category_id), amount=amount,
...
def list_with_spending(db: Session, year: int, month: int,
    ...
    rows = []
    for b in budgets:
        spent = spent_for_category(db, user_id, b.category_id, b.year, b.month)
...
def get_with_spending(db: Session, budget_id: int, user_id: int) -> dict:
    ...
    return row_payload(
        budget, *spent_pct_status(db, user_id, budget)
    )


def spent_pct_status(db: Session, user_id: int, b: Budget):
...
def update_budget(db: Session, budget_id: int, amount: int, user_id: int) -> Budget:
    budget = db.query(Budget).filter(
        Budget.id == budget_id, Budget.user_id == user_id
    ).first()
    if not budget:
